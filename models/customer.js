...
const customerSchema = new Schema(
  {
    firstName: {
      type: String,
      required: [true, 'Tên khách hàng chưa được nhập'],
      maxLength: [50, 'Tên không được vượt quá 50 ký tự'],
    },
    lastName: {
      type: String,
      required: [true, 'Họ khách hàng chưa được nhập'],
    ...
    phoneNumber: {
      ...
      validate: {
        ...
        message: (props) => `${props.value}: phoneNumber của khách hàng không hợp lệ!`,
      },
    },
    address: {
      type: String,
    ...
    email: {
      ...
      validate: {
        validator: function (value) {
          ...
          console.log('<<=== valueEmail ===>>',`${value}: email của khách hàng chưa hợp lệ`);
...
customerSchema.virtual('fullName').get(function () {
  ...
  console.log('<<=== fullName khách hàng ===>>',fullName);
  return fullName;
});
